...
# --- Config ---
st.set_page_config(page_title="Advanced Data Analyst Toolkit — Bulletproof", layout="wide")
sns.set_theme(style="whitegrid")
MAX_PDF_TABLE_ROWS = 50  # limit rows when embedding tables into PDF
...
def df_for_pdf(df):
    # return a clipped copy for safe PDF embedding
    if df.shape[0] > MAX_PDF_TABLE_ROWS:
        return df.head(MAX_PDF_TABLE_ROWS)
    return df


def df_to_table_data_with_index(df, max_rows=MAX_PDF_TABLE_ROWS):
    df2 = df.copy()
...
def add_paragraph(blocks, text):
    blocks.append(('paragraph', str(text)))


def add_table(blocks, df_table):
    blocks.append(('table', df_for_pdf(df_table)))


def add_image_bytes(blocks, bytes_io, title=None):
    # bytes_io is BytesIO or bytes
    blocks.append(('image', bytes_io))
    if title:
        blocks.append(('paragraph', title))


def build_pdf(title, author, analysis_list, blocks):
    buf = io.BytesIO()
    doc = SimpleDocTemplate(buf, pagesize=letter, rightMargin=30, leftMargin=30, topMargin=30, bottomMargin=18)
    styles = getSampleStyleSheet()
    hdr = ParagraphStyle('hdr', parent=styles['Title'], fontSize=18, alignment=1)
    normal = styles['Normal']
    elements = []

    elements.append(Paragraph(title, hdr))
    elements.append(Spacer(1, 8))
    elements.append(Paragraph(f"Author: {author}", normal))
    elements.append(Paragraph(f"Date: {datetime.date.today().isoformat()}", normal))
    elements.append(Spacer(1, 12))
    elements.append(Paragraph("Analyses included:", styles['Heading3']))
    elements.append(Paragraph(", ".join(analysis_list), normal))
    elements.append(Spacer(1, 18))

    for block_type, content in blocks:
        if block_type == 'paragraph':
            elements.append(Paragraph(str(content), normal))
            ...
            try:
                ...
                elements.append(Paragraph(f"(Table render failed: {e})", normal))
                elements.append(Spacer(1, 6))
            ...
            try:
                ...
                elements.append(Paragraph(f"(Image embed failed: {e})", normal))
                elements.append(Spacer(1, 6))
...
# --- UI ---
st.title('Advanced Data Analyst Toolkit — Bulletproof')
st.markdown('Upload a dataset, choose analyses, run them, and generate a PDF containing only selected analyses and embedded charts.')

uploaded = st.file_uploader('Upload CSV / Excel file', type=['csv','xlsx'])

# persistent author
if 'author_name' not in st.session_state:
    st.session_state.author_name = 'Data Analyst'
author_input = st.text_input('Report author (appears on cover)', value=st.session_state.author_name)
st.session_state.author_name = author_input
...
if 'last_chosen' not in st.session_state:
    st.session_state.last_chosen = []

# Run analyses
if st.button('▶ Run selected analyses'):
    st.session_state.pdf_blocks = []
    st.session_state.last_chosen = chosen.copy()
    ...
    if 'Summary Statistics & Missing Values' in chosen:
        ...
        try:
            ...
            st.dataframe(desc)
            add_paragraph(st.session_state.pdf_blocks, 'Summary Statistics (variables + descriptive statistics)')
    ...
    if 'Exploratory Visuals (Numeric & Categorical)' in chosen:
            ...
            try:
                ...
                if annotate:
                    for rect,count in zip(patches,counts):
                        ax.annotate(int(count), xy=(rect.get_x()+rect.get_width()/2, rect.get_height()), xytext=(0,3), textcoords='offset points', ha='center', va='bottom', fontsize=8)
                st.pyplot(fig)
                fig_bytes = fig_to_bytes(fig)
                add_image_bytes(st.session_state.pdf_blocks, fig_bytes, f'Histogram: {col}')
            except Exception as e:
        ...
        for col in explore_ui.get('cat', []):
            try:
                ...
                st.pyplot(fig)
                fig_bytes = fig_to_bytes(fig)
                add_image_bytes(st.session_state.pdf_blocks, fig_bytes, f'Counts: {col}')
            except Exception as e:
    ...
    if 'Correlation Matrix (r & p)' in chosen:
        ...
        if len(cols) <2:
            ...
            try:
                ...
                st.subheader('Pearson r'); st.dataframe(rmat.round(4))
                st.subheader('p-values'); st.dataframe(pmat.round(4))
                ...
                st.pyplot(fig)
                fig_bytes = fig_to_bytes(fig)
                add_image_bytes(st.session_state.pdf_blocks, fig_bytes, 'Correlation heatmap (r)')
            except Exception as e:
    ...
    if 'Simple Linear Regression' in chosen and slr_ui:
        ...
        add_paragraph(st.session_state.pdf_blocks, f'Simple regression: {y} ~ {x}')
        data = df[[x,y]].dropna()
        if data.shape[0] <3:
            ...
            try:
                ...
                add_table(st.session_state.pdf_blocks, coef_df)
                fig, ax = plt.subplots()
                try:
                ...
                st.pyplot(fig)
                fig_bytes = fig_to_bytes(fig)
                add_image_bytes(st.session_state.pdf_blocks, fig_bytes, f'Regression plot: {y} ~ {x}')
            except Exception as e:
    ...
    if 'Multiple Linear Regression' in chosen and mlr_ui.get('X'):
        ...
        add_paragraph(st.session_state.pdf_blocks, f'Multiple regression: {y} ~ {' + ' + '.join(X_list) + '}')
        ...
        if data.shape[0] < (len(X_list)+3):
            ...
            try:
                ...
                st.pyplot(fig)
                fig_bytes = fig_to_bytes(fig)
                add_image_bytes(st.session_state.pdf_blocks, fig_bytes, 'Residuals vs Fitted')
            except Exception as e:
    ...
    if 'Partial Regression (controls)' in chosen and partial_ui and mlr_ui.get('y'):
        ...
        add_paragraph(st.session_state.pdf_blocks, f'Partial regression: effect of {focal} on {y} controlling for {', '.join(controls)}')
        ...
        if sub.shape[0] <4:
            ...
            try:
                ...
                mod1 = sm.OLS(sub[focal], Xc).fit(); res_focal = mod1.resid
                mod2 = sm.OLS(sub[y], Xc).fit(); res_y = mod2.resid
                if res_focal.nunique() <2 or res_y.nunique() <2:
                    r,p = np.nan, np.nan
                else:
                    r,p = stats.pearsonr(res_focal, res_y)
                st.write(f'Partial correlation r={r:.4f}, p={p:.4f}')
                add_paragraph(st.session_state.pdf_blocks, f'Partial correlation r={r:.4f}, p={p:.4f}')
                fig, ax = plt.subplots()
                ...
                try:
                    ...
                    pass
                ax.set_title('Partial regression')
                st.pyplot(fig)
                fig_bytes = fig_to_bytes(fig)
                add_image_bytes(st.session_state.pdf_blocks, fig_bytes, 'Partial regression residuals plot')
            except Exception as e:
    ...
    if 'Regression Diagnostics' in chosen and mlr_ui.get('X') and mlr_ui.get('y'):
        ...
        if data.shape[0] < (len(mlr_ui['X'])+3):
            ...
            try:
                X = sm.add_constant(data[mlr_ui['X']]); model = sm.OLS(data[mlr_ui['y']], X).fit()
                try:
                    from statsmodels.stats.outliers_influence import variance_inflation_factor
                    vif_list = []
                    for i in range(1, X.shape[1]):
                        vif = variance_inflation_factor(X.values, i)
                        vif_list.append({'variable': X.columns[i], 'VIF': round(vif,3)})
                    vif_df = pd.DataFrame(vif_list)
                    st.subheader('Variance Inflation Factors (VIF)'); st.dataframe(vif_df)
    ...
    if 'Compare Means (t-test / ANOVA)' in chosen and means_ui:
        ...
        add_paragraph(st.session_state.pdf_blocks, 'Hypothesis: H0: group means equal; H1: not all means equal.')
        ct = df[[target, group]].dropna(); groups = ct[group].unique()
        ...
        if len(groups) == 2:
            ...
            if len(a) <2 or len(b) <2:
                st.warning('Not enough data for t-test'); add_paragraph(st.session_state.pdf_blocks, 'Not enough data for t-test.')
            else:
                try:
                    t_stat,p = stats.ttest_ind(a,b, nan_policy='omit', equal_var=False)
                    st.write(f't = {t_stat:.4f}, p = {p:.4f}'); add_paragraph(st.session_state.pdf_blocks, f'Independent t-test: t = {t_stat:.4f}, p = {p:.4f}')
                except Exception as e:
                    st.warning(f't-test failed: {e}'); add_paragraph(st.session_state.pdf_blocks, f't-test failed: {e}')
        elif len(groups) >2:
            try:
                formula = f"{target} ~ C({group})"; model = smf.ols(formula, data=ct).fit(); anova_table = anova_lm(model, typ=2)
                st.dataframe(anova_table); add_paragraph(st.session_state.pdf_blocks, 'ANOVA (one-way) — H0: all group means equal; H1: not all equal'); add_table(st.session_state.pdf_blocks, anova_table)
                if 'PR(>F)' in anova_table.columns and anova_table['PR(>F)'].iloc[0] < 0.05:
                    try:
                        res = pairwise_tukeyhsd(ct[target], ct[group]); res_df = pd.DataFrame(data=res._results_table.data[1:], columns=res._results_table.data[0])
                        st.subheader('Tukey HSD post-hoc'); st.dataframe(res_df); add_paragraph(st.session_state.pdf_blocks, 'Tukey HSD post-hoc comparisons'); add_table(st.session_state.pdf_blocks, res_df)
                    except Exception as e:
                        st.warning(f'Tukey HSD failed: {e}'); add_paragraph(st.session_state.pdf_blocks, f'Tukey HSD failed: {e}')
            except Exception as e:
                st.error(f'ANOVA failed: {e}'); add_paragraph(st.session_state.pdf_blocks, f'ANOVA failed: {e}')
    ...
    if 'Compare Variances (Levene)' in chosen and levene_ui:
        ...
        add_paragraph(st.session_state.pdf_blocks, 'H0: group variances equal; H1: not equal')
        sub = df[[target, grp]].dropna(); lev_groups = sub[grp].unique(); arrays = [sub[sub[grp]==g][target].dropna() for g in lev_groups]
        if any(len(a) <2 for a in arrays):
            st.warning('Not enough observations in at least one group for Levene'); add_paragraph(st.session_state.pdf_blocks, 'Not enough observations for Levene.')
        else:
            try:
                stat,p = stats.levene(*arrays); st.write(f'Levene stat = {stat:.4f}, p = {p:.4f}'); add_paragraph(st.session_state.pdf_blocks, f'Levene statistic = {stat:.4f}, p = {p:.4f}')
            except Exception as e:
                st.error(f'Levene failed: {e}'); add_paragraph(st.session_state.pdf_blocks, f'Levene failed: {e}')
    ...
    if 'Association Between Categories (Chi-square)' in chosen and chi_ui:
        ...
        try:
            ct = pd.crosstab(df[a], df[b]); st.subheader('Contingency table'); st.dataframe(ct); add_table(st.session_state.pdf_blocks, ct)
            chi2,p,dof,ex = stats.chi2_contingency(ct); st.write(f'Chi2 = {chi2:.4f}, p = {p:.4f}, dof = {dof}'); add_paragraph(st.session_state.pdf_blocks, f'Chi-square = {chi2:.4f}, p = {p:.4f}, dof = {dof}')
        except Exception as e:
            st.error(f'Chi-square failed: {e}'); add_paragraph(st.session_state.pdf_blocks, f'Chi-square failed: {e}')
...
if st.button('📄 Generate corporate PDF report (analysis-wise)'):
    ...
    elif not st.session_state.author_name or str(st.session_state.author_name).strip() == '':
        st.warning('Please enter the author name in the top field before generating the PDF.')
    else:
        title = 'Data Analysis Report'
        analysis_list = st.session_state.last_chosen if st.session_state.get('last_chosen') else chosen
        pdf_buffer = build_pdf(title, st.session_state.author_name, analysis_list, st.session_state.pdf_blocks)
        st.download_button('📥 Download PDF', data=pdf_buffer, file_name='analysis_report.pdf', mime='application/pdf')
